...
async function main() {
    ...
    try {
        ...
        if (!intelligenceAddress) {
            throw new Error('Intelligence token adresi bulunamadı!');
        }
        console.log(`✅ Intelligence Token: ${intelligenceAddress}\n`);
        ...
        if (!faithAddress) {
            throw new Error('Faith token adresi bulunamadı!');
        }
        console.log(`✅ Faith Token: ${faithAddress}\n`);
        ...
        if (!dexAddress) {
            throw new Error('DEX adresi bulunamadı!');
        }
        console.log(`✅ SimpleDEX: ${dexAddress}\n`);
        
        // 4. Her hesaba 1000 INT ve FTH bas
        console.log('💰 Hesaplara token dağıtılıyor...');
        ...
        for (let i = 0; i < ANVIL_ACCOUNTS.length; i++) {
            ...
            await publicClient.waitForTransactionReceipt({ hash: intHash });
            
            // Faith token bas
            const fthHash = await walletClient.writeContract({
                address: faithAddress,
                abi: ERC20_ABI,
                functionName: 'mintTo',
                args: [account.address, mintAmount]
            });
            await publicClient.waitForTransactionReceipt({ hash: fthHash });
        }
        
        console.log('✅ Tüm hesaplara 1000 INT ve 1000 FTH basıldı!\n');
        
        // 5. .env dosyasını güncelle
        updateEnvFile({
            intelligence: intelligenceAddress,
            faith: faithAddress,
            dex: dexAddress
        });
        ...
        console.log('\n🎉 Kurulum tamamlandı!');
        console.log(`\n📋 Kontrat Adresleri:`);
        console.log(`   Intelligence (INT): ${intelligenceAddress}`);
        console.log(`   Faith (FTH): ${faithAddress}`);
        console.log(`   SimpleDEX: ${dexAddress}`);
        console.log(`\n💡 Şimdi "npm start" ile uygulamayı başlatabilirsiniz!\n`);
